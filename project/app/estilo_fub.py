...
def estilo_passagens(tabela,tamanho):
    ...
    caminho = pegar_caminho(tabela)
    workbook = openpyxl.load_workbook(caminho)
    worksheet = workbook['Passagens e Locomoção']
    ...
    custom_number_format_passagem = []
    # MASCARA R$
    if custom_number_format_passagem!= False: 
        custom_number_format_passagem = NamedStyle(name='custom_number_format_passagem')
        custom_number_format_passagem.number_format = 'R$ #,##0.00'
        custom_number_format_passagem.font = Font(name="Arial", size=12, color="000000")
        custom_number_format_passagem.alignment = Alignment(horizontal="general",vertical="bottom",wrap_text=True)
...
#
    for row in range(start_row,size+1):
        cell = worksheet[f'J{row}']
        cell.style = custom_number_format_passagem
    ...
    #colunas azul
    row_style_passagem = NamedStyle(name='row_style_passagem')
    row_style_passagem.font = Font(name="Arial", size=12, color="FFFFFF",bold=True)
    row_style_passagem.fill = openpyxl.styles.PatternFill(start_color=azul, end_color=azul, fill_type='solid')
    row_style_passagem.alignment = Alignment(horizontal="center",vertical="center",wrap_text=True)
    row_style_passagem.height = 30

    row_number = size + 2
   
    for column in range(1, 11):  
        cell = worksheet.cell(row=row_number, column=column)
        cell.style = row_style_passagem
    ...
    azul_claro = '1c89b8'
    row_style_cabecario_passagem = NamedStyle(name='row_style_cabecario_passagem')
    row_style_cabecario_passagem.font = Font(name="Arial", size=12, color="FFFFFF",bold=True)
    row_style_cabecario_passagem.fill = openpyxl.styles.PatternFill(start_color=azul_claro, end_color=azul_claro, fill_type='solid')
    row_style_cabecario_passagem.alignment = Alignment(horizontal="center",vertical="center",wrap_text = True)
    worksheet.row_dimensions[9].height = 50
    worksheet.column_dimensions['b'].width = 35
    worksheet.column_dimensions['c'].width = 20
    worksheet.column_dimensions['d'].width = 35
    ...
    # Apply the style to each cell in the row
    for cell in worksheet[linha_number]:
        cell.style = row_style_cabecario_passagem
...
def estilo_rendimento_de_aplicacao(tabela,tamanho):
    ...
    coordenadora_cargo_formula = f"='Receita x Despesa'!H46"
    coordenadora_cpf_formula = f"='Receita x Despesa'!H47"
    coordenadora_merge_cells = f'E{coordenadora_row}:F{coordenadora_row}'
    coordenadora_cargo_merge_cells = f'E{coordenadora_cargo_row}:F{coordenadora_cargo_row}'
    coordenadora_cpf_merge_cells = f'E{coordenadora_cpf_row}:F{coordenadora_cpf_row}'
    worksheet.merge_cells(coordenadora_merge_cells)
    worksheet.merge_cells(coordenadora_cargo_merge_cells)
    worksheet.merge_cells(coordenadora_cpf_merge_cells)
